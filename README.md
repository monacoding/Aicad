...
- **자재 추출(BOM)** — 도면의 심볼/파이프에서 **밸브 리스트·파이프(라인) 리스트·장비·
  계기 리스트**를 자동 추출(태그·종류·구경·서비스·길이), 표로 보기 + **CSV 내보내기**.
  상단 "리스트" 버튼 또는 명령행 `BOM` (`src/cad/bom.ts`)
- **자연어 드로잉** — "반지름 4 정육각형 그려줘", "간단한 집 도면" 등을 Claude가
  정밀한 연산 목록으로 변환해 적용
- **이미지 → 도면** — 이미지를 **복사해 Ctrl+V로 붙여넣거나** "🖼 이미지" 버튼으로
  열면 Claude 비전이 인식해 똑같이 작도(선·도형·심볼·텍스트 추적). 로컬 CLI/ API 키 사용
