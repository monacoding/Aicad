...
export type Op =
  ...
  | { op: "add_pipe"; points: XY[]; layer?: string; size?: string; service?: string; tag?: string }
    ...
    for (const op of ops) {
      ...
      switch (op.op) {
        ...
        case "add_pipe": {
          ...
          const es = makePipe(op.points.map(xy), lyr, 1.8, op.size, op.service, op.tag) as Entity[];
          doc.add(...es);
          es.forEach((e) => created.push(e.id));
          break;
        }
